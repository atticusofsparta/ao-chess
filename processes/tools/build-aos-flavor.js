  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        console.error('Failed to start Docker on macOS. Please ensure Docker Desktop is installed and running.', startErr.message);
        process.exit(1);
      }
    } else {
      try {
        ...
        console.error('Failed to start Docker on Linux. Please ensure Docker is installed and you have permissions to start it.', startErr.message);
        process.exit(1);
      }
    }
  }
...
async function main() {
  ...
  const newProcessLua = processLuaParts[0] + '\n' + bundledLua + '\nreturn process' + processLuaParts[1];
